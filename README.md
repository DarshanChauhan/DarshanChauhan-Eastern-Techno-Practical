...
We will build Rest Apis that can create, retrieve, update, delete and find practicals by title.

The following table shows overview of the Rest APIs that will be exported:

- GET     `api/practicals`	            get all practicals
- GET     `api/practicals/:id`         get practical by id
- POST    `api/practicals`             add new practical
- PUT     `api/practicals/:id`         update practical by id
- DELETE  `api/practicals/:id`         remove practical by id
- DELETE  `api/practicals`             remove all practicals
- GET     `api/practicals/published`   find all published practicals
- GET     `api/practicals?title=[kw]`  find all practicals which title contains 'kw'
...
- Create a new practical using `POST /practicals` Api

![node-js-postgresql-crud-example-create](https://dev-to-uploads.s3.amazonaws.com/i/hqvz8ra9p21z927iwzph.png)

After creating some new practicals, you can check PostgreSQL table:
```testdb=# select * from practicals;
